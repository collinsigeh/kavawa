...
class SettingController extends Controller
{
    public function run()
    {
        ...
        foreach($countries_array as $country_key => $country)
        {
            $new_country = [
                'name' => $country->name,
                'iso_code_2' => $country->iso_3166_2,
                'flag' => $country->flag,
                'full_name' => isset($country->full_name) ? $country->full_name : $country->name
            ];
            
             Country::firstOrCreate($new_country);
        }

        $country = Country::where('full_name', 'United States of America')->get();
        $primary_user = User::where('is_primary', 1)->first();

        if(count($country) > 0 && !$primary_user)
        {
            $password = 'ClarityPass';
            ...
            return '
                <p>Setup complete. The primary administrator account was created successfully with the following details: </p>
                <ul>
                    <li>email: <b>'.$user->email.'</b></li>
                    <li>password: <b>'.$password.'</b></li>
                </ul>
                You can now proceed to <a href="'.route('login').'">login</a> OR <a href="'.route('home').'">home page</a>.
            ';
        }

        return 'Setup complete. <a href="'.route('home').'">Return home</a>';
    }
}
